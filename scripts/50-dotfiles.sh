...
# Install Oh My Zsh only if not already installed.[web:149][web:151][web:159]
...
if [[ -f "${HOME}/.zshrc" && ! -f "${HOME}/.zshrc.pre-fedora-setup" ]]; then
  cp "${HOME}/.zshrc" "${HOME}/.zshrc.pre-fedora-setup"
  echo "Backed up existing .zshrc to .zshrc.pre-fedora-setup"
fi
'

  # Set Zsh as default shell for target_user.[web:161][web:163][web:157]
  local zsh_path
...
# Write/overwrite the snippet from the repo file
cat <<EOF_SNIPPET > "${SNIPPET_DEST}"
'"$(sed 's/$/\'$'\n/g' "$ZSH_SNIPPET_SRC" | tr -d '\r')"'
EOF_SNIPPET
...
-- Inspired by common minimal Lua configs.[web:154][web:156][web:162]
...
Performed (depending on your choices):
  - Installed Zsh and Oh My Zsh for your user, and attempted to set Zsh as the default shell.[web:149][web:151][web:159][web:161][web:163]
  - Installed your Zsh snippet under ~/.zsh.d and wired it into ~/.zshrc.
  - Created a minimal Neovim init.lua at ~/.config/nvim/init.lua if none existed.[web:154][web:156][web:162]
  - Set some safe global Git defaults (init.defaultBranch=main, color.ui=auto, optional editor and delta pager).
